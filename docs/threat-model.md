...
| A macOS binary ships without the hardening its target is assumed to give it | before anything is signed, the release reads `MH_PIE` off `otool -h` and confirms no local symbols remain (`nm` shows only `U`) for each Mach-O binary; rustc emits PIE on Apple targets by default and the release profile strips local symbols, so a missing property is a toolchain or linker regression the release refuses to publish | test: `tests/shell/check-hardening-macos.test.sh`, one control per property built with `clang` (`-Wl,-no_pie` for the PIE control, an unstripped link for the other); the script runs on both macOS legs of the release; the test skips where `clang`/`otool` are absent so it does not block the Linux CI lane |
